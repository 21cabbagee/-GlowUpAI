...
class GrowthFeatureTests(unittest.TestCase):
    def setUp(self):
        ...
        settings = Settings(db_path=Path(self.temp.name) / "growth.sqlite3", photo_dir=None, gemini_api_key=None, gemini_enabled=False)
    ...
    def test_commerce_offers_are_not_premium_gated(self):
        ...
        offer = self.client.post("/api/admin/offers", json={"product_id": product["id"], "merchant": "Acme", "url": "https://example.com/p", "price_cents": 1500})
    ...
    def test_budget_optimizer_is_premium_gated_and_estimates_cost_for_stable_keep(self):
        ...
        offer = self.client.post("/api/admin/offers", json={"product_id": product["id"], "merchant": "Acme", "url": "https://example.com/p", "price_cents": 3000})
        self.assertEqual(offer.status_code, 200)
